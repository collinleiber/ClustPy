...
def create_subspace_data(n_samples=1000, n_clusters=3, cluster_features=2, total_features=5, std=1, box=(-10, 10)):
    assert total_features >= cluster_features, "total_features can not be smaller than cluster_features"
    X, L = create_nr_data(n_samples=n_samples, n_clusters=[n_clusters], cluster_features=[cluster_features],
                          total_features=total_features, std=std, box=box)
...
def create_nr_data(n_samples=1000, n_clusters=[3, 3], cluster_features=[2, 2], total_features=5, std=1, box=(-10, 10)):
    ...
    for i in range(len(n_clusters)):
        X_tmp, L_tmp = make_blobs(n_samples, cluster_features[i], centers=n_clusters[i], cluster_std=std,
                                  center_box=box)
        X1 = np.c_[X1, X_tmp]
        L = np.c_[L, L_tmp]
