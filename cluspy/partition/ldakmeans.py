...
def _lda_kmeans(X: np.ndarray, n_clusters: int, n_dims: int, max_iter: int, kmeans_repetitions: int,
                random_state: np.random.RandomState) -> (np.ndarray, np.ndarray, np.ndarray):
    """
    ...
    return km.labels_, rotation, km.cluster_centers_
...
class LDAKmeans(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, n_clusters: int, n_dims: int = None, max_iter: int = 300, kmeans_repetitions: int = 10,
    ...
    def fit(self, X: np.ndarray, y: np.ndarray = None) -> 'LDAKmeans':
        ...
        """
        labels, rotation, centers = _lda_kmeans(X, self.n_clusters, self.n_dims, self.max_iter, self.kmeans_repetitions,
                                                self.random_state)
        self.labels_ = labels
