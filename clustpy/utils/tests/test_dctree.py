...
def test_dc_tree_min():
    X = np.array([[0.0, 0.0], [-3.0, 0.0], [2.0, 0.0], [3.0, 0.0], [-7.0, 0.0], [-8.0, 0.0]])
    # Min_points = 3
    ...
    expected_distances = np.array(
        [
            ...
            [4.0, 0.0, 4.0, 4.0, 4.0, 5.0],
            [3.0, 4.0, 0.0, 3.0, 4.0, 5.0],
            [3.0, 4.0, 3.0, 0.0, 4.0, 5.0],
    ...
    expected_distances = np.array(
        [
            [0.0, 3.0, 2.0, 2.0, 4.0, 4.0],
            [3.0, 0.0, 3.0, 3.0, 4.0, 4.0],
            [2.0, 3.0, 0.0, 1.0, 4.0, 4.0],
            [2.0, 3.0, 1.0, 0.0, 4.0, 4.0],
            [4.0, 4.0, 4.0, 4.0, 0.0, 1.0],
            [4.0, 4.0, 4.0, 4.0, 1.0, 0.0],
        ]
    )
    assert np.array_equal(all_distances, expected_distances)
    all_distances_reverse = dctree.dc_distances(np.arange(6), np.arange(5)[::-1], access_method="dc_dist")
...
def test_reachability_distances():
    ...
    expected_distances = [
        [0.0, 4.0, 3.0, 3.0, 7.0, 8.0],
        [4.0, 0.0, 5.0, 6.0, 4.0, 5.0],
        [3.0, 5.0, 0.0, 3.0, 9.0, 10.0],
        [3.0, 6.0, 3.0, 0.0, 10.0, 11.0],
        [7.0, 4.0, 9.0, 10.0, 0.0, 5.0],
        [8.0, 5.0, 10.0, 11.0, 5.0, 0.0],
    ]
    assert np.array_equal(distances, expected_distances)
...
def test_minimum_spanning_tree_prims():
    ...
    expected_mst = np.array(
        [(0, 2, 2.0), (2, 3, 1.0), (0, 1, 3.0), (1, 4, 4.0), (4, 5, 1.0)],
        dtype=([("i", int), ("j", int), ("dist", float)]),
    )
