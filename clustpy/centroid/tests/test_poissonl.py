...
def test_poissonl_estimator():
    check_clustpy_estimator(PoissonL(3, allow_negative_values=True), ("check_complex_data"))


def test_poissonc_estimator():
    check_clustpy_estimator(PoissonC(3, allow_negative_values=True), ("check_complex_data"))
...
def test_simple_PoissonL():
    ...
    assert poissonl.column_lambdas_.shape == (poissonl.n_clusters, X.shape[1])
    assert len(np.unique(poissonl.labels_)) == poissonl.n_clusters
    assert np.array_equal(np.unique(poissonl.labels_), np.arange(poissonl.n_clusters))
    ...
    poissonl2.fit(X)
    assert np.array_equal(poissonl.n_clusters, poissonl2.n_clusters)
...
def test_simple_PoissonC():
    ...
    assert poissonc.column_lambdas_.shape == (poissonc.n_clusters, X.shape[1])
    assert len(np.unique(poissonc.labels_)) == poissonc.n_clusters
    assert np.array_equal(np.unique(poissonc.labels_), np.arange(poissonc.n_clusters))
    ...
    poissonc2.fit(X)
    assert np.array_equal(poissonc.n_clusters, poissonc2.n_clusters)
