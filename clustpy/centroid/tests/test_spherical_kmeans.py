...
def test_simple_SphericalKMeans():
    ...
    assert skm.cluster_centers_.shape == (skm.n_clusters, X.shape[1])
    assert len(np.unique(skm.labels_)) == skm.n_clusters
    assert np.array_equal(np.unique(skm.labels_), np.arange(skm.n_clusters))
    labels_predict = skm.predict(X)
    assert np.array_equal(skm.labels_, labels_predict)
    # Test if random state is working
    skm2 = SphericalKMeans(3, random_state=1)
    skm2.fit(X)
    assert np.array_equal(skm.n_clusters, skm2.n_clusters)
    assert np.array_equal(skm.labels_, skm2.labels_)
    assert np.array_equal(skm.cluster_centers_, skm2.cluster_centers_)
