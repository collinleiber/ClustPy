...
from clustpy.deep._utils import run_initial_clustering, detect_device, mean_squared_error
...
def get_neural_network(input_dim: int, embedding_size: int = 10, neural_network: _AbstractNeuralNetwork | tuple[type[_AbstractNeuralNetwork], dict] | None = None,
                        neural_network_class: type[_AbstractNeuralNetwork] = FeedforwardAutoencoder,
                        neural_network_params: dict | None = None, neural_network_weights: str | Path | None = None, device : torch.device | None = None,
                        random_state: np.random.RandomState | int | None = None) -> _AbstractNeuralNetwork:
    ...
    neural_network : _AbstractNeuralNetwork | tuple[type[_AbstractNeuralNetwork], dict] | None
        ...
        If it is a tuple, it has to consist of the neural network class (_AbstractNeuralNetwork) and the initialization parameters (dict).
    ...
    neural_network_class : type[_AbstractNeuralNetwork]
    ...
    if neural_network is not None and isinstance(neural_network, tuple):
        assert len(
    ...
    if neural_network is None:
        ...
        neural_network_instance = neural_network_class(**neural_network_params)
    else:
        assert isinstance(neural_network, _AbstractNeuralNetwork), "neural_network must be of type _AbstractNeuralNetwork. Your type: {0}".format(type(neural_network))
        neural_network_instance = neural_network
    assert hasattr(neural_network_instance,
    ...
    if neural_network_weights is not None:
        neural_network_instance.load_parameters(neural_network_weights)
    if neural_network_instance.work_on_copy:
        # If neural network is used by multiple deep clustering algorithms, create a deep copy of the object
        neural_network_instance = copy.deepcopy(neural_network_instance)
    # Move neural network to device
    device = detect_device(device)
    neural_network_instance.to(device)
    return neural_network_instance


def get_trained_network(trainloader: torch.utils.data.DataLoader | None = None, data: np.ndarray | None = None,
                        n_epochs: int = 100, batch_size: int = 128, optimizer_params: dict | None = None,
                        optimizer_class: type[torch.optim.Optimizer] = torch.optim.Adam, device: torch.device | None = None,
                        ...
                        neural_network_class: type[_AbstractNeuralNetwork] = FeedforwardAutoencoder,
                        neural_network_params: dict | None = None, neural_network_weights: str | Path | None = None,
                        random_state: np.random.RandomState | int | None = None) -> _AbstractNeuralNetwork:
...
def get_default_deep_clustering_initialization(X: np.ndarray | torch.Tensor, n_clusters: int | None, batch_size: int,
                                               pretrain_optimizer_params: dict | None, pretrain_epochs: int,
                                               optimizer_class: type[torch.optim.Optimizer],
                                               ssl_loss_fn: Callable | torch.nn.modules.loss._Loss,
                                               neural_network: _AbstractNeuralNetwork | tuple[type[_AbstractNeuralNetwork], dict] | None, embedding_size: int,
                                               ...
                                               initial_clustering_params: dict | None, device: torch.device | int | str | None,
                                               random_state: np.random.RandomState,
                                               neural_network_class: type[_AbstractNeuralNetwork] = FeedforwardAutoencoder,
                                               neural_network_params: dict | None = None,
                                               neural_network_weights: str | Path | None = None) -> tuple[
        torch.device, torch.utils.data.DataLoader, torch.utils.data.DataLoader, int, _AbstractNeuralNetwork, np.ndarray, int,
    ...
    Returns
    -------
    tuple : tuple[torch.device, torch.utils.data.DataLoader, torch.utils.data.DataLoader, int, _AbstractNeuralNetwork, np.ndarray, int,
        np.ndarray, np.ndarray, ClusterMixin]
        The device,
        The trainloader,
        The testloader,
