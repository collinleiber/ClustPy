...
from clustpy.deep.autoencoders._masked_autoencoder_utils import get_2d_sincos_pos_embed

from clustpy.deep.autoencoders._abstract_autoencoder import _AbstractAutoencoder

class MaskedAutoencoder(_AbstractAutoencoder):
    ...
    def decode(self, x, ids_restore=None):
        # embed tokens
    ...
    def loss(self, batch: list, loss_fn: torch.nn.modules.loss._Loss, device: torch.device) -> (
        ...
        target = self.patchify(batch_data)
        if self.norm_pix_loss:
            mean = target.mean(dim=-1, keepdim=True)
            var = target.var(dim=-1, keepdim=True)
            target = (target - mean) / (var + 1.e-6)**.5

        embedded, mask, ids_restore = self.encode_masked(batch_data)
        reconstructed = self.decode(embedded, ids_restore)  # [N, L, p*p*3]
        # TODO: Need to pass loss_fn that does not sum over all patches
    ...
    def forward(self, imgs):
        embedded, mask, ids_restore = self.encode_masked(imgs)
        reconstruction = self.decode(embedded, ids_restore)  # [N, L, p*p*3]
