from clustpy.deep._utils import squared_euclidean_distance, detect_device, encode_batchwise, predict_batchwise, window, \
    int_to_one_hot, decode_batchwise, encode_decode_batchwise, run_initial_clustering
...
def test_run_initial_clustering():
    ...
    # Test with GMM
    n_clusters, labels, centers, clustering_algo = run_initial_clustering(X, 3, GaussianMixture, {"n_init":3}, random_state)
    assert n_clusters == 3
    assert labels.shape == L.shape
    assert centers.shape == (n_clusters, X.shape[1])
