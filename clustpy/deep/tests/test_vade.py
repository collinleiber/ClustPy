...
def test_simple_vade_with_nrletters():
    ...
    vade = VaDE(6, pretrain_epochs=3, clustering_epochs=3, n_gmm_initializations=3, random_state=1)
    assert not hasattr(vade, "labels_")
    vade.fit(X)
    assert vade.labels_.dtype == np.int32
    assert vade.labels_.shape == labels.shape
    # Test if random state is working
    vade2 = VaDE(6, pretrain_epochs=3, clustering_epochs=3, n_gmm_initializations=3, random_state=1)
    vade2.fit(X)
