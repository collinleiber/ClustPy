...
def test_neighbor_encoder():
    ...
    dataloader = get_dataloader(data, 30, True, additional_inputs=neighbors)
    neighborencoder = NeighborEncoder(layers=[data.shape[1], 10, embedding_dim], n_neighbors=n_neighbors,
                                      decode_self=False)
    # Test encoding
    ...
    loss, embedded = neighborencoder.loss(first_batch, loss_fn, torch.device("cpu"))
    assert loss.item() >= 0
    ...
    n_neighbors = 2
    neighborencoder_2 = NeighborEncoder(layers=[data.shape[1], 10, embedding_dim], n_neighbors=n_neighbors,
                                        decode_self=True)
...
def test_neighbor_encoder_in_deep_clustering():
    ...
    neighbor_ids = np.argsort(dist_matrix, axis=1)
    neighbors = [data[neighbor_ids[:, 1 + i]] for i in range(n_neighbors)]
    trainloader = get_dataloader(data, 30, True, additional_inputs=neighbors)
    testloader = get_dataloader(data, 30, False, additional_inputs=neighbors)
