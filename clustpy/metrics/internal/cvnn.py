...
def cvnn_score(X: np.ndarray, labels: np.ndarray | int | tuple, n_neighbors: int = 5, metric: str = "euclidean", noise_strategy: str = "keep") -> float | np.ndarray:
    """
    ...
    if isinstance(labels, list) or isinstance(labels, tuple):
        ...
        for i, l in enumerate(labels):
            cluster_separation_l, cluster_compactness_l = _internal_cvnn_score(X, l, nrbs_indices, metric, noise_strategy)
        ...
        cluster_separation, cluster_compactness = _internal_cvnn_score(X, labels, nrbs_indices, metric, noise_strategy)
